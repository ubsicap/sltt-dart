...
# optional: run the project's test setup to ensure Isar native libs are available
# run this from repo root (not inside packages/sltt_core)
# source ./setup_test_env.sh && PUB_CACHE="$PUB_CACHE" dart test packages/sltt_core 2>&1 | tee test-sltt_core-offline.log
