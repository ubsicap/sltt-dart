...
import 'package:aws_backend/src/models/note_comment_emoji_reacted.data.dart';
...
void main() {
  group(
    'offline - Entity State - Serialization/Deserialization - DynamoNoteCommentEmojiReactedDataEntityState',
    () {
      test(
        ...
        () {
          ...
          final changeLogEntry =
              ...
              >(
                ...
                entityId: 'comment-reaction-drift-test',
                entityType: kEntityTypeCommentReaction,
                domainId: 'project1',
                domainType: 'project',
                changeAt: localChangeAt,
                cid: 'cid-comment-reaction-drift-test',
                changeBy: 'user1',
                data: data,
                operation: 'create',
                dataSchemaRev: 0,
          ...
          if (testEntityState.unknownJson != '{}') {
            ...
            SlttLogger.logger.info(
              'DEBUG: Action needed: Either add missing fields to BaseEntityState or update DynamoNoteCommentEmojiReactedDataEntityState',
            );
            fail(
              'FIELD DRIFT DETECTED: DynamoNoteCommentEmojiReactedDataEntityState has unknown fields:\n${const JsonEncoder.withIndent(' ').convert(testEntityState.getUnknown())}',
            );
          }
          ...
          expect(
            ...
            equals('{}'),
            reason:
                'DynamoNoteCommentEmojiReactedDataEntityState should deserialize without unknown fields',
          );
          ...
          expect(
            testEntityState.data_emoji,
            equals('👍'),
            reason: 'emoji field should be correctly deserialized',
          );
          expect(
          ...
          expect(
            testEntityState.data_emoji_changeAt_,
            equals(localChangeAt.toUtc()),
            reason: 'data_emoji_changeAt_ should be in UTC',
          );
            ...
            updates.stateUpdates.forEach((stateKey, stateValue) {
              if (key == stateKey ||
                ...
                expect(
                  ...
                  isNull,
                  reason:
                      'Field "$key" has "null" value but stateUpdates["$stateKey"] has non-null value ($stateValue) in DynamoNoteCommentEmojiReactedDataEntityState',
                );
              }
            });
          });
          ...
          expect(
            ...
            reason:
                'DynamoNoteCommentEmojiReactedDataEntityState should include all optional fields with null values',
          );
        },
      );
    },
