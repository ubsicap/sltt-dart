...
class SyncSystemDemo {
  ...
  Future<void> _demonstrateOutsyncFlow() async {
    ...
    for (final change in updateChanges) {
      await _dio.post('$_outsyncsUrl/api/changes', data: [change]);
      print(
        '   ✅ Added change: ${change['operation']} ${change['entityType']} (${change['entityId']})',
      );
    }
  ...
  Future<void> _demonstrateDownsyncFlow() async {
    ...
    for (final change in remoteUpdates) {
      await _dio.post('$_cloudStorageUrl/api/changes', data: [change]);
      print(
        '   ✅ Added remote change: ${change['operation']} ${change['entityType']} (${change['entityId']})',
      );
    }
  ...
  Future<void> _demonstrateFullSyncFlow() async {
    ...
    print(
      '   ✅ Added final change to Outsyncs: update Settings (app-settings-final)',
    );
